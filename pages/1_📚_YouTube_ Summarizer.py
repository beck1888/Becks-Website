try:
    ...
    # Check if the error is related to missing session state attributes
    if "st.session_state has no attribute" in str(e):
        # Clear the session state to reset the app
        st.session_state = {}
        # Rerun the app to reinitialize the session state
        st.rerun()
